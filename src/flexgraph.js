...
export function getValueDisplay(sortedData, styles) {
  ...
  let label = getTextSVG(
    "displaytext",
    display,
    [33, sortedData.padTop * 0.4 + "%"],
    styles.displayFontSize,
    styles.displayColor,
    styles.displayFontWeight
  );
  return <svg key="valuedisplay">{label}</svg>;
}

export function getRectangleSVG(
  key,
  styles,
  mouseDown,
  mouseOut,
  ...
  return (
    <rect
      key={key}
      x={styles.topLeftPoint[0] + "%"}
      y={styles.topLeftPoint[1] + "%"} //array [x,y]
      width={styles.width + "%"}
...
export function GetCircleSvg(
  ...
  styles.heightMultiplier =
    parseFloat(styles.canvasWidth) / parseFloat(styles.canvasHeight);
...
export function getPathSVG(
  ...
  const svgPath = (points, command) => {
    ...
    return (
      <path
        key={key}
        style={{ width: 7, strokeDasharray: dashSize }}
        d={d}
        fill={fill}
        stroke={color}
        strokeWidth={strokeWidth}
  ...
  const lineCommand = (point, i, a) => {
    if (smoothing) {
      ...
      return `C ${cpsX},${cpsY} ${cpeX},${cpeY} ${point[0]},${
        point[1] / styles.heightMultiplier
      }`;
    } else {
  ...
  if (combinedData.drawFree === false) {
    for (let set in combinedData.sortedData) {
      ...
      for (let pair in sortedData.xAscending) {
        combinedData.sortedData[set].drawArray.push([
          (sortedData.xAscending[pair][0] - combinedData.xMin) *
            combinedData.xMultiplier +
            combinedData.padLeft,
          combinedData.yLimit -
            (sortedData.xAscending[pair][1] - combinedData.yMin) *
              combinedData.yMultiplier +
            combinedData.padTop,
        ]);
      }
    }
    ...
    for (let i in data) {
      ...
      if (data[i].length) {
        for (let pair in data[i]) {
          combinedData.sortedData[set].drawArray.push([
            (data[i][pair][0] - combinedData.xMin) * combinedData.xMultiplier +
              combinedData.padLeft,
            combinedData.yLimit -
              (data[i][pair][1] - combinedData.yMin) *
                combinedData.yMultiplier +
              combinedData.padTop,
          ]);
        }
...
export function getXAxisSVG(sortedData, styles) {
  ...
  let xLine = getPathSVG(
    "xLine",
    styles,
    [
      [sortedData.padLeft, 100 - sortedData.padTop],
      [100 - sortedData.padLeft, 100 - sortedData.padTop],
    ],
    styles.axisColor,
    styles.axisLineSize
  );
  ...
  for (let i = 0; i <= styles.xTicks; i++) {
    ...
    textArray.push(
      getTextSVG(
        "xrulerValue" + i,
        Math.round(sortedData.xMin + rulerStep * i),
        [
          rulerPosition + rulerOffset * i + "%",
          100 - sortedData.padTop / 2 + "%",
        ],
        styles.fontSize,
        styles.fontColor
      )
    ...
    if (i >= 1) {
      tickArray.push(
        getPathSVG(
          ...
          styles,
          [
            [rulerPosition + rulerOffset * i, 100 - sortedData.padTop],
            [rulerPosition + rulerOffset * i, sortedData.padTop],
          ],
          styles.tickColor,
          styles.tickLineSize,
          0,
  ...
  textArray.push(
    getTextSVG(
      "xNameText",
      styles.xName,
      [100 - middleX / 1.8, 100 - sortedData.padTop * 1.2 + "%"],
      styles.fontSize,
      styles.labelColor
    )
  );
...
export function getYAxisSVG(sortedData, styles) {
  ...
  for (let i = 0; i <= styles.yTicks; i++) {
    ...
    let intLeft = () => {
      if (roundInt.length <= 10) {
        return roundInt.length / 2;
      } else {
        return 5;
      }
    };
    let tenthLeft = () => {
      if (roundTenth.length <= 10) {
        return roundTenth.length / 2;
      } else {
        return 5;
      }
    };
    ...
    if (sortedData.yDiff > 50) {
      ...
      textArray.push(
        getTextSVG(
          ...
          roundTenth,
          [
            sortedData.padLeft / 2 - tenthLeft() + "%",
            rulerPosition - rulerOffset * i + "%",
          ],
          styles.fontSize,
          styles.fontColor
    ...
    if (i >= 1) {
      tickArray.push(
        getPathSVG(
          ...
          styles,
          [
            [sortedData.padLeft, rulerPosition - rulerOffset * i],
            [100 - sortedData.padLeft, rulerPosition - rulerOffset * i],
          ],
          styles.tickColor,
          styles.tickLineSize,
          0,
  ...
  let yLine = getPathSVG(
    "yLine",
    styles,
    [
      [sortedData.padLeft, 100 - sortedData.padTop],
      [sortedData.padLeft, sortedData.padTop],
    ],
    styles.axisColor,
    styles.axisLineSize
  );
  ...
  let label = getTextSVG(
    "ylabeltext",
    styles.yName,
    [-middleX / 1.8 / styles.heightMultiplier, sortedData.padLeft * 1.25],
    styles.fontSize,
    styles.labelColor
  );
...
export function getmarkerLine(sortedData, styles) {
  if (sortedData.yMin < 0 && sortedData.yMax > 0) {
    let y =
      sortedData.yLimit -
      (0 - sortedData.yMin) * sortedData.yMultiplier +
      sortedData.padTop;

    let range = [[], []];
    range = [
      [sortedData.padLeft, y],
      [100 - sortedData.padLeft, y],
    ];
    let path = getPathSVG(
      "markerLine",
      styles,
      range,
      styles.markerLineColor,
      styles.markerLineSize,
      false,
      1
    );
    let text = getTextSVG(
...
export function GraphPoints(key, sortedData, styles) {
  ...
  for (let set in sortedData) {
    ...
    if (
      !styles.type ||
      sortedData[set].type === "Mark" ||
      sortedData[set.type] === "LineMark"
    ) {
      for (let pair in sortedData[set].drawArray) {
        ...
        // }
        if (
          currentPair[0] === styles.selectedPoint[0] &&
          currentPair[1] === styles.selectedPoint[1]
        ) {
          color = styles.clickPointColor;
          //alreadySelected = true;
        }
        if (
          currentPair[0] === selectedPoint.value[0] &&
          currentPair[1] === selectedPoint.value[1] &&
        ...
        let mouseOver = () => {
          handlePointClick(xVal, yVal, xDraw, yDraw);
        };
        // let mouseExit = () => setHovered([]);
        let mouseExit = () => {
          setSelectedPoint({ value: [], draw: [] });
          globalStyles.clickedPoint = null;
        };
...
export function drawCanvas(key, styles, Sketch) {
  ...
  return (
    <div
      ...
      style={{
        position: "absolute",
        top: styles.canvasPadTop,
        left: styles.canvasPadLeft,
    ...
    >
      <svg
        key={key + "canvasBox"}
        style={{ background: styles.canvasColor }}
        viewBox={ViewBox}
      >
...
export function LineMarkGraph(data, styles) {
  ...
  for (let set in sortedData) {
    ...
    if (
      !styles.type ||
      sortedData[set].type === "Line" ||
      sortedData[set.type] === "LineMark"
    ) {
      Path = getPathSVG(
        "graphPath" + set,
        styles,
        sortedData[set].drawArray,
        sortedData[set].color,
        styles.lineSize
      );
      Paths.push(Path);
    }
  ...
  let canvas = drawCanvas("LineMarkCanvas", styles, [
    markerLine,
    XAxis,
    YAxis,
    Paths,
...
export function FlexButton(key, styles, mouseDown, mouseHover, mouseExit) {
  ...
  let buttonClickedStyle = {
    ...
    text: {
      placement: [styles.btnTextRange[0], styles.btnTextRange[1] + 0.75],
    },
  };
  ...
  let buttonCover = getRectangleSVG(
    "flexButtoncover",
    btn.cover,
    mouseDownAction,
    mouseExit,
...
export function DrawShapesGraph(data, styles) {
  ...
  let combinedData = sortXYArray(data, styles, 80, 80, true, [
    [0, 0],
    [100, 100],
  ]);
...
function GetDisplayPop(styles, selectedPoint) {
  ...
  if (globalStyles.clickedPoint) {
    topLeftPoint = [
      globalStyles.clickedPoint.draw[0] - 6,
      globalStyles.clickedPoint.draw[1] - 29,
    ];
    ...
    if (flip === false) {
      pointy = getPathSVG(
        ...
        styles,
        [
          [topLeftPoint[0] + 4.5, topLeftPoint[1] + displayPop.height - 0.1],
          [topLeftPoint[0] + 7.5, topLeftPoint[1] + displayPop.height - 0.1],
      ...
      pointy = getPathSVG(
        ...
        styles,
        [
          [topLeftPoint[0] + 4.5, topLeftPoint[1] + 0.1],
          [topLeftPoint[0] + 7.5, topLeftPoint[1] + 0.1],
    ...
    yVal =
      commaFormat(globalStyles.clickedPoint.value[1].toString()) +
      styles.ySymbol;
    if (yVal.length > 8) {
    ...
    xHeaderXPosition =
      topLeftPoint[0] +
      5.75 -
      (xHeaderFontSize / 3.11) * (styles.popXDisplay.toString().length - 1) +
      "%";
    yHeaderXPosition =
      topLeftPoint[0] +
      5.75 -
      (yHeaderFontSize / 3.11) * (styles.popYDisplay.toString().length - 1) +
      "%";

    xValueXPosition =
      topLeftPoint[0] +
      5.75 -
      (xValueFontSize / 3.11) * (xVal.toString().length - 1) +
      "%";
    yValueXPosition =
      topLeftPoint[0] +
      5.75 -
      (yValueFontSize / 3.11) * (yVal.toString().length - 1) +
      "%";

    //calc specific if statement, remove first part later
    let irrFontSize = 2;
    let irrHeaderXPosition =
      topLeftPoint[0] +
      5.75 -
      (irrFontSize / 3.11) * (xVal.toString().length - 1) +
      "%";
    if (globalStyles.clickedPoint.value[1] === 0) {
      text.push(
        getTextSVG(
          ...
          [
            topLeftPoint[0] +
              5 -
              (xHeaderFontSize / 3.11) *
                (styles.popXDisplay.toString().length - 1) +
              "%",
            topLeftPoint[1] + 6 + "%",
          ],
          2,
      ...
      text.push(
        getTextSVG(
          "Xvalue",
          xVal,
          [irrHeaderXPosition, topLeftPoint[1] + 12 + "%"],
          irrFontSize,
          styles.background,
          "bold"
        )
    ...
    //keep this part, remove from if statement
      ...
      text.push(
        getTextSVG(
          "Xname",
          styles.popXDisplay,
          [xHeaderXPosition, topLeftPoint[1] + 4 + "%"],
          xHeaderFontSize,
          styles.background,
          styles.displayFontWeight
        )
      ...
      text.push(
        getTextSVG(
          "Xvalue",
          xVal,
          [xValueXPosition, topLeftPoint[1] + 8 + "%"],
          xValueFontSize,
          styles.background,
          "bold"
        )
      ...
      text.push(
        getTextSVG(
          "yname",
          styles.popYDisplay,
          [yHeaderXPosition, topLeftPoint[1] + 12 + "%"],
          yHeaderFontSize,
          styles.background,
          styles.displayFontWeight
        )
      ...
      text.push(
        getTextSVG(
          "yvalue",
          yVal,
          [yValueXPosition, topLeftPoint[1] + 16 + "%"],
          yValueFontSize,
          styles.background,
          "bold"
        )
